...
public class SearchController implements Initializable {
	...
	public void Search(ActionEvent e) {
		...
		String sea = ss.getText();
		PreparedStatement ps7;
		ResultSet rs7;
		String query5 = "select * from books where book_id = ?";   // where book_id = ?
		
		conc = MySqlConnectionAdmin.MySqlConnector();
		try {
					
			ps7 = conc.prepareStatement(query5);
			ps7.setString(1, sea);
			rs7 = ps7.executeQuery();
			
			while(rs7.next()) {
				list.add(new BooksData(rs7.getString(1),rs7.getString(2),rs7.getString(3),rs7.getString(4),rs7.getString(5)));
			}
