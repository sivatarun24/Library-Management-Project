...
public class AddUserBookController implements Initializable {
	...
	public void Add(ActionEvent e) throws Exception {
		...
		String query4 = "insert into bookswithusers (user_id , book_id) values (? , ?)";
		...
		if(i > 0) {
			try {
				...
				Stage stage = new Stage();
				FXMLLoader loader = new FXMLLoader();
				Pane root = loader.load(getClass().getResource("/application/DialogUserBook.fxml").openStream());
				Scene scene = new Scene(root);
				scene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
				stage.setScene(scene);
				stage.show();
